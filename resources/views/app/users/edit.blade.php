                    ...
                    <div
                        ...
                        <div>
                            <div class="inline-flex gap-x-2">
                                ...
                                <a href="{{ route('app.users.destroy', $user) }}" data-turbo-method="delete"
                        ...
                        <form action="{{ route('app.users.update', $user) }}" method="post">
                            ...
                            <div class="mb-3">
                                <x-form.label>Name</x-form>
                                    <x-form.input name="name" type="text" :value="$user->name" class="w-full" />
                            </div>


                            <div class="mb-3">
                                <x-form.label>Email</x-form>
                                    <x-form.input name="email" type="email" :value="$user->email" class="w-full" />
                            </div>
                                ...
                                <x-form.label>Roles</x-form>
                                    <select data-choices-target="element" multiple name="role_ids[]">
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->id }}" @selected($user->hasRole($role))>
                                                {{ $role->name }}
                                            </option>
                                        @endforeach
                                    </select>
